...
if(isset($_POST["user_view"]) && strlen($_POST["user_view"])>0 && isset($_POST["view_user_id"]) && strlen($_POST["view_user_id"])>0){
	...
	if(!empty($_POST["user_email"])){	
		$condition[]="email='".$_POST["user_email"]."'";
	}
	
	if(!empty($_POST["user_number"])){	
		$condition[]="mobile='".$_POST["user_number"]."'";
	}
	
	if($_POST["user_status"]==1 || $_POST["user_status"]=='0'){	
		$condition[]="status='".$_POST["user_status"]."'";
	}
	$condition[]="admin_id='$user_id'";
	 $htl_qry="SELECT login.id,`username`, `email`, `mobile`, `country`, `password`, `at_home`, `status`, `admin_id` FROM `login` LEFT OUTER JOIN administrator ON administrator.id =login.admin_id  where ".implode(" && ",$condition)." order by login.id desc";
         //echo $htl_qry;
 ...
 $query=$DB->runQuery($conn,$htl_qry);
		echo '<table class="table table-striped">
									...
									foreach($query as $val){
										$status=($val["status"]==1)?'<a href="#" data-hotel_id="'.$val["id"].'" title="Admin Active" class="btn btn-xs btn-success" id="hotel_status" data-status="'.$val["status"].'"><i class="fa  fa-plus-circle"></i> Active</a>':'<a href="#" data-hotel_id="'.$val["id"].'" title="Admin Active" class="btn btn-xs btn-danger" id="hotel_status" data-status="'.$val["status"].'"><i class="fa  fa-minus-circle"></i> In-active</a>';
										$at_home=($val["at_home"]=='1')?' checked':'';
										echo '<tr><td>'.$cnt.'</td>';										
										echo '<td>'. $val["username"].'</td><td>'.$val["email"].'</td><td>'.$val["mobile"].'</td><td >'.$status.'</td><td><input type="checkbox"'.$at_home.' data-hotel_id="'.$val["id"].'" id="at_home" /></td><td><a title="view" class="btn btn-xs btn-warning" id="user_view" data-hotel_id="'.$val["id"].'" href="#myModal" data-toggle="modal"><i class="fa fa-file-o"></i> View</a></td><td><a href="user-add?token='.$val["id"].'" title="Edit" class="btn btn-xs btn-danger"><i class="fa fa-pencil-square-o"></i> Edit</a></td>
										';
										if($user_id==2){
...
if(isset($_POST["hotel_id"]) && strlen($_POST["hotel_id"])>0 && is_numeric($_POST["hotel_id"]) && isset($_POST["hotel_userid"]) && strlen($_POST["hotel_userid"])>0  && is_numeric($_POST["hotel_userid"]) ){
	...
	if(!empty($select)){
	if($hotel_status==0){
	$query=$DB->updateQuery($conn,"UPDATE `login` SET `status`='1' WHERE `id`='$hotel_id' && `admin_id`='$user_id'");	
	}else{
	$query=$DB->updateQuery($conn,"UPDATE `login` SET `status`='0' where id='$hotel_id' && admin_id='$user_id'");		
	}
	if($query){
		echo 1;
	} else{
